...
class PyraformerAttention(nn.Module):
    def __init__(self, Pyraformer_model, emb_dim, out_dim, device):
        super(PyraformerAttention, self).__init__()
        self.device = device

        self.Pyraformer_model = Pyraformer_model
        self.fc1 = nn.Linear(emb_dim + emb_dim, emb_dim)
        self.tanh = nn.Tanh()
        self.fc2 = nn.Linear(emb_dim, out_dim)

    def forward(self, src, index, max_len):
        output, hidden = self.Pyraformer_model(src)
