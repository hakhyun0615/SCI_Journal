...
class ODE_Attention(nn.Module):
    ...
    def forward(self, eco_data, tran_data, tran_data_label, time_f, alpha):
        # 경제데이터
        _, eco_vector = self.eco_model(eco_data)
        ...
        for i in range(len(tran_data.shape[1])):
            Q[:,i,:] = self.Q_W(tran_data[:,i,:],time_f[:,i,:]) + eco_vector*alpha
            K[:,i,:] = self.K_W(tran_data[:,i,:],time_f[:,i,:]) + eco_vector*alpha
            V[:,i,:] = self.V_W(tran_data[:,i,:],time_f[:,i,:]) + eco_vector*alpha
