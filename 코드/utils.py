...
# 전처리
def preprocess(transaction_df, economy_df, window_size=5):
    # transaction_all 전처리
    ...
    transaction_df['제곱미터당 거래금액'] = (transaction_df['거래금액(만원)'] / transaction_df['전용면적(㎡)']) / 100 ########
    transaction_df['단지'] = transaction_df['시군구'] + ' ' + transaction_df['단지명']
    transaction_df = transaction_df[['단지', '계약년월', '건축년도', '제곱미터당 거래금액']]
    ...
    transaction_df = transaction_df.groupby(['단지','계약년월','건축년도'])['제곱미터당 거래금액'].mean().to_frame().reset_index()

    # 2022년도 전까지 거래량 적은(window_size+1 미만) 단지 제거
    transaction_else_df = transaction_df[transaction_df['계약년월']//100 != 2022]
    apartment_complex_volume = transaction_else_df.groupby('단지').agg({'건축년도':'first','제곱미터당 거래금액':'count'}).reset_index().rename(columns={'제곱미터당 거래금액':'거래량'})
    ...
    # 이상치 제거
    transaction_df['Z 스코어'] = transaction_df.groupby('단지')['제곱미터당 거래금액'].transform(lambda x: (x-x.mean()) / x.std())
    ...
    transaction_df = transaction_df[['동','단지','건축년도','계약년월','제곱미터당 거래금액']]

    return transaction_df, economy_df
