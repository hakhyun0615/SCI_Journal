...
class Dong_Dataset(Dataset):
    def __init__(self, model, table_1, table_2, table_3, embedding_dim, window_size, DEVICE):
        ...
        dongs_apartment_complexes_embedding_matrixes_with_window_size = [] # (전체 동 개수 * 204-window_size, 38, window_size, embedding_dim) # (22698, 38, 10, 1024)
        ...
        dongs_apartment_complexes_prices_with_window_size = [] # (전체 동 개수 * 204-window_size, 38, 1)
        ...
        for dong in dongs: 
            ...
            economy_values = table_2_copy[['call_rate','m2']].values # 경제 지표 값들 (204, 2)
            economy_tensor = torch.FloatTensor(economy_values).to(DEVICE) # 경제 지표 텐서 변환

            encoder_input_tensors = torch.zeros(dong_apartment_complexes_values.shape[0], 204, 12).to(DEVICE) # 인코더 입력 텐서들 초기화(인코더 입력 텐서 여러개) # (동 안의 단지 개수, 204(시점), 12)
            for i, dong_apartment_complex_values in enumerate(dong_apartment_complexes_values):
                dong_apartment_complex_tensor = torch.FloatTensor(dong_apartment_complex_values).to(DEVICE).repeat(204,1) 
            ...
            dong_apartment_complexes_prices = torch.zeros(dong_apartment_complexes_aids.shape[0],204,1).to(DEVICE) # (동 안의 단지 개수, 204, 1)
            for i, dong_apartment_complex_aid in zip(range(dong_apartment_complexes_aids.shape[0]), dong_apartment_complexes_aids): # 동 안의 단지 개수, 동 안의 단지들의 aids
                dong_apartment_complexes_prices[i] = torch.from_numpy(pd.DataFrame({'did': range(0, 204)}).merge(table_3_copy[table_3_copy['aid'] == dong_apartment_complex_aid][['did','price']], on='did', how='outer').fillna(0).set_index('did').values) # (204, 1)
            ...
            for i in range(204-window_size): # window_size 고려한 시점(0~199)
                ...
                dong_apartment_complexes_prices_with_window_size = torch.zeros(max_apartment_complexes, 1).to(DEVICE) # (38, 1)
