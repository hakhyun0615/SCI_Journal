...
class Test_Dataset(Dataset):
    def __init__(self, model, table_1, table_2, table_3, embedding_dim, window_size, DEVICE):
        ...
        # 동별 최대 단지 개수
        max_apartment_complexes = max(table_1_copy.groupby('dong')['name'].count()) # 24

        dongs_apartment_complexes_embedding_matrixes_with_window_size = [] # (전체 동 개수 * 204-window_size, 38, window_size, embedding_dim) # (1120, 24, 10, 1024)
        dongs_apartment_complexes_embedding_matrixes_with_window_size_num = [] # 단지 개수 # (전체 동 개수 * 204-window_size, 1)
        dongs_apartment_complexes_embedding_matrixes_with_window_size_index = [] # y 값이 있는 단지 index # (전체 동 개수 * 204-window_size, ?)
        dongs_apartment_complexes_prices_with_window_size = [] # (전체 동 개수 * 204-window_size, 24, 1)
        ...
        for dong in dongs: 
            ...
            economy_values = table_2_copy[['call_rate','m2']].values # 경제 지표 값들 (20, 2)
            economy_tensor = torch.FloatTensor(economy_values).to(DEVICE) # 경제 지표 텐서 변환

            encoder_input_tensors = torch.zeros(dong_apartment_complexes_values.shape[0], 20, 12).to(DEVICE) # 인코더 입력 텐서들 초기화(인코더 입력 텐서 여러개) # (동 안의 단지 개수, 20(시점), 12)
            for i, dong_apartment_complex_values in enumerate(dong_apartment_complexes_values):
                dong_apartment_complex_tensor = torch.FloatTensor(dong_apartment_complex_values).to(DEVICE).repeat(20,1) 
            ...
            with torch.no_grad():
                dong_apartment_complexes_embedding_matrixes = torch.zeros(encoder_input_tensors.shape[0], 20, embedding_dim) # (동 안의 단지 개수, 20, 1024)
                for i in range(encoder_input_tensors.shape[0]): # 동 안의 단지 (20, 1024)
                    apartment_complex_embedding_matrix = torch.zeros(20,embedding_dim) # (20, 1024)
                    for j in range(20): # 시점
            ...
            # dong_apartment_complexes_prices(동 안의 단지마다 가격 구한 뒤 리스트 형식으로 모으기) 완성 # (동 안의 단지 개수, 20, 1)
            dong_apartment_complexes_aids = table_1_copy[table_1_copy['dong'] == dong]['aid'].values # (동 안의 단지 개수, )
            dong_apartment_complexes_prices = torch.zeros(dong_apartment_complexes_aids.shape[0], 20, 1).to(DEVICE) # (동 안의 단지 개수, 20, 1)
            for i, dong_apartment_complex_aid in zip(range(dong_apartment_complexes_aids.shape[0]), dong_apartment_complexes_aids): # 동 안의 단지 개수, 동 안의 단지들의 aids
                dong_apartment_complexes_prices[i] = torch.from_numpy(pd.DataFrame({'did': range(0, 20)}).merge(table_3_copy[table_3_copy['aid'] == dong_apartment_complex_aid][['did','price']], on='did', how='outer').fillna(0).set_index('did').values) # (20, 1)
            
            # dong_apartment_complexes_embedding_matrixes와 dong_apartment_complexes_prices window_size로 나누기
            for i in range(20-window_size): # window_size 고려한 시점(0~199)
                dong_apartment_complexes_embedding_matrixes_with_window_size = torch.zeros(max_apartment_complexes, window_size, embedding_dim) # (24, window_size, 6)
                dong_apartment_complexes_prices_with_window_size = torch.zeros(max_apartment_complexes, 1).to(DEVICE) # (24, 1)
                ...
                dongs_apartment_complexes_embedding_matrixes_with_window_size.append(dong_apartment_complexes_embedding_matrixes_with_window_size) # (24, window_size, 6)
                dongs_apartment_complexes_prices_with_window_size.append(dong_apartment_complexes_prices_with_window_size) # (24, 1)
